...
{
    public function apis_call(Request $request)
    {
        return view('apis_call');
    }

    public function load_apis_log_for_table(Request $request)
    {
        if ($request->search["value"]) {
            $keyword = $request->search["value"];
            $user_result = apis_log::where(function ($query) use ($keyword) {
                $query->where('api_call', "like", "%" . $keyword . "%")
                    ->orWhere('status',  "like", "%" . $keyword . "%");
            })->get();
        } else {
            $user_result = apis_log::orderBy('id', 'desc')->get();
        }
